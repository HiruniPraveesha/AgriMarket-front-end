...
export default function AddProduct() {
  ...
  const fetchSellerDetails = async (sellerId: string) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/sellers/${mySeller}`
      );
  ...
  const handleFileChange = async (
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      if (!isValid) {
        setModalMessage(
          "Please select an image with dimensions 271x186 and file type JPEG, JPG, or PNG."
        );
        setShowModal(true);
        return;
      }
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response) {
          ...
          setModalMessage(
            `Error adding product: ${axiosError.response.data.error}`
          );
        } else {
