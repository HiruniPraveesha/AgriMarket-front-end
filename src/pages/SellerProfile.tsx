...
import ProfileImage from "../assets/Carrot.png"; // Add a placeholder image for the seller profile
...
const SellerProfile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8001/api/seller/details?sellerId=${sellerId}") 
  ...
  const handleShowProducts = () => {
    fetch("http://localhost:8001/api/seller/products?sellerId=${sellerId}")
      .then((response) => response.json())
      .then((data) => {
        const productsWithRatings = data.map(
          (product: { review: string | any[] }) => ({
            ...product,
            rating:
              product.review.length > 0 ? product.review[0].rating : "N/A", // Assuming you want to show the first rating found
          })
        );
