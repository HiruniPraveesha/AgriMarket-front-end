...
import { Container, Dropdown, Modal, Button, Form, Row, Col } from 'react-bootstrap';
...
const ManageProductPage: React.FC<{}> = ({}) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/products/seller/${mySeller}`);
  ...
  const updateProduct = async (productId: number, updatedProduct: any) => {
    try {
      const response = await axios.put(`http://localhost:8000/products/${productId}`, updatedProduct);
      ...
      <Row>
        ...
        <Col>
          <div style={{ padding: '5% 15% 5%' }}>
            ...
            <p style={{ fontSize: '15px', fontStyle: 'bold', margin: '12px' }}>{storeName}</p>
            <hr />
                    ...
                    <td style={{ position: 'relative' }}>
                      <Dropdown>
                        ...
                        <Dropdown.Menu style={{ zIndex: 9999 }}>
                          <Dropdown.Item onClick={() => handleEdit(product.product_id, product.name, product.price, product.description)}>Update</Dropdown.Item>
              ...
              <Modal.Body>
                <Form>
                  <Form.Group controlId="formProductId">
                    ...
                    <Form.Control type="text" placeholder="Enter product ID" value={editProduct.id} readOnly />
                  </Form.Group>
                  <Form.Group controlId="formProductName">
                    <Form.Label style={{ fontWeight: 'bold', color: 'black' }}>Product Name</Form.Label>
                    <Form.Control type="text" placeholder="Enter product name" value={editProduct.name} onChange={(e) => setEditProduct({ ...editProduct, name: e.target.value })} />
                  </Form.Group>
                  <Form.Group controlId="formCurrentPrice">
                    <Form.Label style={{ fontWeight: 'bold', color: 'black' }}>Current Price</Form.Label>
                    <Form.Control type="text" placeholder="Enter current price" value={editProduct.price} readOnly />
                  </Form.Group>
                  <Form.Group controlId="formNewPrice">
                    <Form.Label style={{ color: '#00BA29' }}>New Price</Form.Label>
                    <Form.Control type="number" placeholder="Enter new price" value={newPrice !== null ? newPrice : ''} onChange={(e) => setNewPrice(parseFloat(e.target.value))} />
                  </Form.Group>
                  <Form.Group controlId="formCurrentPrice">
                    <Form.Label style={{ fontWeight: 'bold', color: 'black' }}>Current Description</Form.Label>
                    <Form.Control type="text" placeholder="Enter current price" value={editProduct.description} readOnly />
                  </Form.Group>
                  <Form.Group controlId="formNewDescription">
                    <Form.Label style={{ color: '#00BA29' }}>New Description</Form.Label>
                    <Form.Control as="textarea" placeholder="Enter new description" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} />
                  </Form.Group>
                </Form>
              </Modal.Body>
              <Modal.Footer>
                ...
                <Button variant="primary" onClick={handleEditConfirmation} style={{backgroundColor: '#00BA29', borderColor: '#00BA29'}}>
                  Save Changes
                </Button>
              </Modal.Footer>
            </Modal>
