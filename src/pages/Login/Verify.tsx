...
const Verify: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const otp = code.join("");
      const response = await axios.post("http://localhost:8001/verify-otp", { otp });
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response && axiosError.response.data) {
          ...
          const errorMessage = typeof responseData === "string" ? responseData : "An error occurred while verifying OTP.";
          ...
          {code.map((value, index) => (
            <div key={index} className="d-inline-block px-1" style={{ margin: '15px' }}>
              <input
                ...
                style={{ width: "50px", height: "60px", fontSize: "16px", backgroundColor: '#BEF3C9', border: 'none', borderBottom: '2px solid #00BA29', borderRadius: '0' }}
                value={value}
