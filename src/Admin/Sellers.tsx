...
import { Container, Row, Col, Button, Dropdown, DropdownButton, FormControl } from 'react-bootstrap';
...
const SellerRow: React.FC<SellerRowProps> = ({ seller_id, email, contactNo, store_name, onDelete }) => {
  ...
  return (
    <>
       <Row
        className={`mb-4 d-flex justify-content-between align-items-center ${selected ? 'bg-light' : ''}`}
      ...
      >
        ...
        <Col md="1" lg="1" xl="1">
          <div style={{ position: 'relative', width: '30px', height: '30px', borderRadius: '50%', overflow: 'hidden' }}>
            <img src={Photo} alt="Photo" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', top: '0', left: '0' }} />
          </div>
        </Col>

        <Col md="5" lg="5" xl="5" className="d-flex align-items-center">
          <p className="mb-0" style={{ fontSize: '10px' }}>{email}</p>
        </Col>

        <Col md="3" lg="3" xl="3" className="d-flex align-items-center">
          <p className="mb-0" style={{ fontSize: '10px' }}>{contactNo}</p>
        </Col>

        <Col md="1" lg="1" xl="1">
          <DropdownButton id="dropdown-basic-button" variant="light" title="">
            <Dropdown.Item onClick={toggleExpanded} style={{ fontSize: '10px' }}>View Details</Dropdown.Item>
            <Dropdown.Item onClick={handleDelete} style={{ fontSize: '10px' }}>Remove Buyer</Dropdown.Item>
          </DropdownButton>
        </Col>
      </Row>
      {expanded && (
        <Row className="mb-3">
          <Col md={{ span: 2, offset: 2 }} style={{ fontSize: '10px' }}>
            <p><strong>Name:</strong> {store_name}</p>
          </Col>
...
const Sellers: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSellers = async () => {
      try {
        const response = await axios.get('http://localhost:8080/admin/getAllSellers');
  ...
  return (
    <AdminLayout>
      <Container>
        <Typography variant="h5" style={{ fontWeight: 'bold', color: '#258544', marginLeft: '280px' }}>
          Manage Sellers
        </Typography>
        ...
        <Row>
          ...
          <Col md="10">
            <Row className="mb-4 d-flex justify-content-between align-items-center" style={{ fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', color: 'gray' }}>
              <Col md="1" lg="2" xl="2">
            ...
            {filteredSellers.map((seller) => (
              <SellerRow key={seller.seller_id} seller_id={seller.seller_id} email={seller.email} contactNo={seller.contactNo} store_name={seller.store_name} onDelete={handleDelete}/>
            ))}
          </Col>
