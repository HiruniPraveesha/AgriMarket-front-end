...
const ProductRow: React.FC<ProductRowProps> = ({ sellerId, product_id, name, quantity, store_name, onDelete }) => {
    ...
    <>
      ...
      >
        ...
        <Col md="5" lg="5" xl="5" className="d-flex align-items-center">
          <p className="mb-0" style={{ fontSize: '10px' }}>{name}</p>
        </Col>

        <Col md="3" lg="3" xl="3" className="d-flex align-items-center">
          <p className="mb-0" style={{ fontSize: '10px' }}>{sellerId}</p>
        </Col>
        ...
        <Col md="1" lg="1" xl="1">
          <DropdownButton id="dropdown-basic-button" variant="light" title="">
            ...
            <Dropdown.Item onClick={handleDelete} style={{ fontSize: '10px' }}>Remove Product</Dropdown.Item>
          </DropdownButton>
        </Col>
      </Row>
      {expanded && (
...
const Products: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('http://localhost:8080/admin/getAllProductsData');
  ...
  const filteredProducts = products.filter(
    product =>
      (product.store_name && product.store_name.toLowerCase().includes(search.toLowerCase())) ||
